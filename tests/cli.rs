...
fn exclude_newer_selects_latest_installed_patch_from_real_rig() {
    ...
    let cache_dir = test_cache("ir-exclude-newer-r-selection-cache");
    fs::write(
        &script,
        format!(
            concat!(
    ...
    assert_success(&out);
    assert_stdout_contains(&out, "ir.fixture=exclude-newer-r-selection");
    ...
    write_executable(
        ...
        &format!(
            concat!(
                ...
                "  cat <<'JSON'\n",
                r#"[{{"name":"4.5.0","version":"4.5.0","date":"2025-04-11"}},{{"name":"4.5.3","version":"4.5.3","date":"2026-03-11"}},{{"name":"4.6.0","version":"4.6.0","date":"2026-04-24"}}]"#,
                "\nJSON\n",
                "  exit 0\n",
                "fi\n",
                "if [ \"$1\" = available ] && [ \"$2\" = --json ]; then\n",
                "  cat <<'JSON'\n",
                r#"[{{"name":"4.5.3","version":"4.5.3","date":"2026-03-11"}},{{"name":"4.6.0","version":"4.6.0","date":"2026-04-24"}}]"#,
                "\nJSON\n",
                "  exit 0\n",
                "fi\n",
...
fn exclude_newer_refreshes_stale_available_cache_for_installed_r_release() {
    ...
    let rig_dir = unique_dir("ir-exclude-newer-stale-rig");
    let script = unique_path("ir-exclude-newer-stale-cache", "R");

    let r45_dir = rig_dir.join("4.5.0");
    let r46_dir = rig_dir.join("4.6.0");
    fs::create_dir_all(&r45_dir).unwrap();
    fs::create_dir_all(&r46_dir).unwrap();

    write_executable(
        &r45_dir.join("Rscript"),
        concat!(
            "#!/bin/sh\n",
            "if [ -n \"${IR_RESOLVE_RESULT_FILE:-}\" ]; then\n",
            "  : > \"$IR_RESOLVE_RESULT_FILE\"\n",
    ...
    fs::write(
        ...
        r#"[{"name":"4.5.0","version":"4.5.0","date":"2025-04-11"}]"#,
    )
    .unwrap();
    ...
    write_executable(
        ...
        &format!(
            concat!(
                ...
                "if [ \"$1\" = list ] && [ \"$2\" = --json ]; then\n",
                "  cat <<'JSON'\n",
                r#"[{{"name":"4.5.0","version":"4.5.0","aliases":[],"binary":"{}/R"}},{{"name":"4.6.0","version":"4.6.0","aliases":[],"binary":"{}/R"}}]"#,
                "\nJSON\n",
                "  exit 0\n",
                "fi\n",
                "if [ \"$1\" = available ] && [ \"$2\" = --all ] && [ \"$3\" = --json ]; then\n",
                "  cat <<'JSON'\n",
                r#"[{{"name":"4.5.0","version":"4.5.0","date":"2025-04-11"}},{{"name":"4.6.0","version":"4.6.0","date":"2026-04-24"}}]"#,
                "\nJSON\n",
                "  exit 0\n",
                "fi\n",
                "echo unexpected rig command: \"$@\" >&2\n",
    ...
    assert_success(&out);
    assert_stdout_contains(&out, "selected=4.6.0");
...
#[cfg(unix)]
#[test]
fn exclude_newer_skips_incomplete_rig_list_entries() {
    let cache_dir = unique_dir("ir-exclude-newer-incomplete-cache");
    let bin_dir = unique_dir("ir-exclude-newer-incomplete-bin");
    let rig_dir = unique_dir("ir-exclude-newer-incomplete-rig");
    let script = unique_path("ir-exclude-newer-incomplete", "R");

    let r453_dir = rig_dir.join("4.5.3");
    fs::create_dir_all(&r453_dir).unwrap();
    write_executable(
        &r453_dir.join("Rscript"),
        concat!(
            "#!/bin/sh\n",
            "if [ -n \"${IR_RESOLVE_RESULT_FILE:-}\" ]; then\n",
            "  : > \"$IR_RESOLVE_RESULT_FILE\"\n",
    ...
    write_executable(
        ...
        &format!(
            concat!(
                ...
                "if [ \"$1\" = list ] && [ \"$2\" = --json ]; then\n",
                "  cat <<'JSON'\n",
                r#"[{{"name":"broken-version","version":null,"aliases":[],"binary":"/missing/R"}},{{"name":"broken-binary","version":"4.6.0","aliases":[],"binary":null}},{{"name":"4.5.3","version":"4.5.3","aliases":[],"binary":"{}/R"}}]"#,
                "\nJSON\n",
                "  exit 0\n",
                "fi\n",
                "echo unexpected rig command: \"$@\" >&2\n",
    ...
    assert_success(&out);
    assert_stdout_contains(&out, "selected=4.5.3");
...
#[cfg(unix)]
#[test]
fn exclude_newer_skips_installed_r_without_rscript() {
    let cache_dir = unique_dir("ir-exclude-newer-broken-rscript-cache");
    let bin_dir = unique_dir("ir-exclude-newer-broken-rscript-bin");
    let rig_dir = unique_dir("ir-exclude-newer-broken-rscript-rig");
    let script = unique_path("ir-exclude-newer-broken-rscript", "R");

    let r453_dir = rig_dir.join("4.5.3");
    let r460_dir = rig_dir.join("4.6.0");
    fs::create_dir_all(&r453_dir).unwrap();
    fs::create_dir_all(&r460_dir).unwrap();
    write_executable(
        &r453_dir.join("Rscript"),
        concat!(
            "#!/bin/sh\n",
    ...
    write_executable(
        ...
        &format!(
            concat!(
                ...
                "if [ \"$1\" = list ] && [ \"$2\" = --json ]; then\n",
                "  cat <<'JSON'\n",
                r#"[{{"name":"4.5.3","version":"4.5.3","aliases":[],"binary":"{}/R"}},{{"name":"4.6.0","version":"4.6.0","aliases":[],"binary":"{}/R"}}]"#,
                "\nJSON\n",
                "  exit 0\n",
                "fi\n",
                "echo unexpected rig command: \"$@\" >&2\n",
...
#[cfg(unix)]
#[test]
fn exclude_newer_ignores_broken_rscript_before_refreshing_available() {
    let cache_dir = unique_dir("ir-exclude-newer-broken-refresh-cache");
    let bin_dir = unique_dir("ir-exclude-newer-broken-refresh-bin");
    let rig_dir = unique_dir("ir-exclude-newer-broken-refresh-rig");
    let script = unique_path("ir-exclude-newer-broken-refresh", "R");

    let r453_dir = rig_dir.join("4.5.3");
    let r470_dir = rig_dir.join("4.7.0");
    fs::create_dir_all(&r453_dir).unwrap();
    fs::create_dir_all(&r470_dir).unwrap();
    write_executable(
        &r453_dir.join("Rscript"),
        concat!(
            "#!/bin/sh\n",
    ...
    write_executable(
        ...
        &format!(
            concat!(
                ...
                "if [ \"$1\" = list ] && [ \"$2\" = --json ]; then\n",
                "  cat <<'JSON'\n",
                r#"[{{"name":"4.5.3","version":"4.5.3","aliases":[],"binary":"{}/R"}},{{"name":"4.7.0","version":"4.7.0","aliases":[],"binary":"{}/R"}}]"#,
                "\nJSON\n",
                "  exit 0\n",
                "fi\n",
                "if [ \"$1\" = available ] && [ \"$2\" = --all ] && [ \"$3\" = --json ]; then\n",
