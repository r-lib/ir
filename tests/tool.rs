...
#[test]
fn tool_run_trims_env_exclude_newer_before_resolution_cache_keying() {
    let cache_dir = unique_dir("ir-tool-env-exclude-newer-normalized-cache");
    let library = unique_dir("ir-tool-env-exclude-newer-normalized-library");
    let profile = unique_path("ir-tool-env-exclude-newer-normalized-profile", "R");
    let entered = unique_path("ir-tool-env-exclude-newer-normalized-entered", "txt");
    ...
    fs::write(
        exec_dir.join("hello.R"),
        r#"#!/usr/bin/env Rscript
cat("tool.fixture=normalized-exclude-newer-cache\n")
"#,
    )
    .unwrap();
    fs::write(
...
if (nzchar(Sys.getenv("IR_RESOLVE_RESULT_FILE"))) {
  ...
  library_path <- Sys.getenv("IR_TEST_LIBRARY")
  writeLines(library_path, Sys.getenv("IR_RESOLVE_RESULT_FILE"))
  ...
  if (nzchar(marker)) {
    ...
    exclude_newer <- trimws(Sys.getenv("IR_EXCLUDE_NEWER"))
    source <- if (nzchar(exclude_newer)) {
      paste0("exclude-newer: ", exclude_newer)
    } else {
      paste0("latest: ", as.integer(Sys.time()))
    }
    writeLines(c(source, library_path), marker)
  }

  package_marker <- Sys.getenv("IR_PRIMARY_PACKAGE_MARKER")
  if (nzchar(package_marker)) {
    writeLines("irfake", package_marker)
  }
...
"#,
    ...
    for _ in 0..2 {
        let out = ir()
            ...
            .args(["tool", "run", "--from", "irfake", "hello"])
            .output()
            .unwrap();

        assert_success(&out);
        assert_stdout_contains(&out, "tool.fixture=normalized-exclude-newer-cache");
    }

    let resolver_runs = fs::read_to_string(&entered).unwrap_or_default();
    assert_eq!(
        resolver_runs.lines().count(),
        1,
        "second tool run should reuse the Rust warm resolution cache"
    );
