...
pub(crate) fn latest_minor_version_on(exclude_newer: &str) -> Result<String, Box<dyn Error>> {
    ...
    if exclude_newer > latest_embedded_minor_release_date() {
        if let Some(discovered) = latest_available_minor_version_on(exclude_newer)? {
            version = Some(
                version
...
fn latest_embedded_minor_release_date() -> &'static str {
    EMBEDDED_MINOR_RELEASES
        .iter()
        .map(|release| release.date)
        .max()
        .expect("embedded R minor releases are generated from nonempty release metadata")
}
