...
fn tool_install_recovery_command(
    ...
    let mut words = vec![
        ...
        "--force".to_string(),
        "--bin-dir".to_string(),
        recovery_command_word(&launcher_path_str(&install.bin_dir)?),
    ];
    ...
    } else if install.rscript.is_some() {
        words.push("--rscript".to_string());
        words.push(recovery_command_word(&rscript.to_string_lossy()));
    } else if let Some(req) = env_r_version() {
        words.push("--r-version".to_string());
        words.push(recovery_command_word(&req));
    } else if nonempty_env("IR_RSCRIPT").is_some() {
        words.push("--rscript".to_string());
        words.push(recovery_command_word(&rscript.to_string_lossy()));
    }
...
fn env_r_version() -> Option<String> {
    env::var("IR_R_VERSION").ok().and_then(|value| {
        let value = value.trim();
...
fn installed_launcher_contents(
    ...
    lines.extend([
        format!("IR_SELECTED_RSCRIPT={}", sh_quote_os(rscript)),
        "if [ ! -x \"$IR_SELECTED_RSCRIPT\" ]; then".to_string(),
        "  printf '%s\\n' \"ir: missing selected R executable: $IR_SELECTED_RSCRIPT\" >&2"
            .to_string(),
        format!(
            "  printf '%s\\n' {} >&2",
            sh_quote_str(&format!(
...
fn installed_launcher_contents(
    ...
    let runtime_check = format!(
        "set \"IR_SELECTED_RSCRIPT={}\"\r\n\
         if not exist \"%IR_SELECTED_RSCRIPT%\" (\r\n\
         echo ir: missing selected R executable: %IR_SELECTED_RSCRIPT% 1>&2\r\n\
         echo ir: run `{recovery_command}` to recreate this tool. 1>&2\r\n\
         exit /b 1\r\n\
         )\r\n",
        rscript.to_string_lossy().replace('"', "\"\"")
    );
