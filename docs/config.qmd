...
| `IR_RSCRIPT` | runs without `r-version` or `exclude-newer` | `Rscript` on `PATH` |
...
`IR_RSCRIPT` is ignored when `r-version` or `exclude-newer` is set, because version selection is resolved through `rig`.
