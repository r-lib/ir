...
The installers download the latest release archive and install `ir` into `~/.local/bin` on Unix or `$HOME\bin` on Windows. Set `IR_INSTALL_DIR` to choose another directory.
