...
- Select R with `--rscript`, `--r-version`, environment variables, frontmatter, or date-only `exclude-newer`.
...
- Install persistent launchers for `exec/` tools and symlinks for opaque `bin/` executables with `ir tool install`.
